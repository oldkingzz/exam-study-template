...
**Step 5 — Write the lesson.** One card per inventory line, in an order that (a) respects prerequisites and (b) meets the homework's concepts in the order the homework asks them. Follow the card anatomy and the **HTML-native rules** in [pedagogy.md](pedagogy.md); use the skeleton, tab markup and widget conventions in [html-guide-template.md](html-guide-template.md). The page must do what markdown cannot: every 记/推 card carries a **visual** (an interactive widget with sliders or draggable points, an SVG diagram, or a comparison table) and its text is cut to bullets within the budget. Every card carries its tag; every card with an `HW hit` carries a `考点` badge naming the question. Each lecture opens with a one-screen *overview strip* and ends with a *回头看* recap and a *常见坑* pitfalls block. Done when every inventory line has a card, every 记/推 card has a visual, a worked example and a self-check, and no card's visible text exceeds the budget.
...
**Step 8 — Read it as the student.** Open the file (browser or screenshot) and read one full lecture section plus one solution as someone who has never seen the topic. Fix any place where a symbol appears before its meaning, or a step is skipped. Done when that read-through needs no outside knowledge.
