...
**Step 6 — Write the solutions, bilingual.** Each `.solution` has two layers. (a) A **walkthrough in the lesson language** inside `<div class="zh">` (clearly labelled "not for submission"): what the question is actually asking in everyday words, why it is asked, the two or three ideas it combines, and a tiny concrete instance — the same plain-language rule as the lesson. (b) The **submittable working in the course's submission language** (English): restate → link the card(s) → work it fully → box the final answer → one line on *why this is what they wanted*; **no lesson-language characters outside `.zh`**, because this part is copied into the submission as-is. Open the section with a one-paragraph map ("you have already met everything you need: Q1 ← cards …"). Multiple-choice: give the verdict for **every** option. Done when each solution has a `.zh` walkthrough, links to ≥1 existing card id, states a final answer, and the gate's CJK check is clean.
...
Lesson prose in the language the user writes in; technical terms in English at first mention with a gloss in that language, then whichever is shorter. **Solutions are bilingual**: a lesson-language walkthrough in `.zh` blocks, then the submittable working in the course's submission language (English for most courses) with no lesson-language characters outside `.zh` (that part gets submitted as-is). Formulas in MathJax, never as images. The tag chips and tab labels in the template are Chinese (记 / 推 / 了解, 算一遍 / 现在哪里用 / 自测); swap the visible labels for the user's language, keep the class names the gate reads.
